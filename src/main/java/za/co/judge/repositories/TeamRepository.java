...
public interface TeamRepository extends Neo4jRepository<Team, Long> {
    @Query("MATCH (m:TeamController)-[r:TEAM_MEMBER]->(a:Member) WHERE toLower(m.name) CONTAINS toLower({name}) RETURN a")
    Collection<Member> getTeamMembers(@Param("name") String name);

    @Query("MATCH (m:TeamController)-[r:SUBMITTED]->(a:Submission) WHERE toLower(m.name) CONTAINS toLower({name}) RETURN a")
    Collection<Submission> getSubmissionsForTeam(@Param("name") String name);

    @Query("MATCH (m:TeamController) WHERE toLower(m.name) CONTAINS toLower({name}) RETURN m")
